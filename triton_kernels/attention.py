...
@triton.jit
def _direct_sparse_decode_kernel(
    Q,
    KV,
    Indices,
    Lengths,
...
def direct_sparse_attention(
    ...
    _direct_sparse_decode_kernel[grid](
        q,
        buffer,
        indices,
...
@triton.jit
def _direct_dual_sparse_decode_kernel(
    Q,
    KV0,
    KV1,
    Indices0,
...
def direct_dual_sparse_attention(
    ...
    attn_sink_tensor = attn_sink if has_attn_sink else lse[:1]
    grid = lambda meta: (total_tokens, triton.cdiv(h_q, meta["BLOCK_H"]))
    _direct_dual_sparse_decode_kernel[grid](
        q,
        primary_buffer,
        extra_buffer,
        primary_indices,
        primary_lengths.to(torch.int32) if primary_lengths.dtype not in (torch.int32, torch.int64) else primary_lengths,
        extra_indices,
        extra_lengths.to(torch.int32) if extra_lengths.dtype not in (torch.int32, torch.int64) else extra_lengths,
        attn_sink_tensor,
        output,
        lse,
        sm_scale,
